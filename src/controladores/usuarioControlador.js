...
const registrarUsuario = async (req, res) => {
  try {
    const { error } = validadorRegistro.validate(req.body, {
      abortEarly: false,
    });

    if (error) {
      const mensajesErrores = error.details
    ...
    if (usuarioExistente) {
      return res
        .status(400)
        .json({ mensaje: 'El usuario ya existe', resultado: null });
    }
...
const actualizarUsuario = async (req, res) => {
  try {
    ...
    if (!usuario) {
      return res
        .status(404)
        .json({ mensaje: 'Usuario no encontrado', resultado: null });
    }

    const nuevoUsuario = await Usuario.update(
      { email, nombre, edad },
      { where: { cedula } },
    );
    res
      .status(200)
      .json({ mensaje: 'Usuario actualizado', resultado: nuevoUsuario });
  } catch (error) {
    res.status(500).json({ mensaje: error.message, resultado: null });
  }
};
...
const borrarUsuario = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ mensaje: 'Usuario eliminado', resultado: borrarUsuario });
  } catch (error) {
    res.status(500).json({ mensaje: error.message, resultado: null });
  }
};
